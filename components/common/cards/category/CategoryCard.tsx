...
import dynamicStyles from './categoryCard.style';
...
const CategoryCard: React.FC<Props> = ({ item, handleHeartPress, isLiked, index }) => {
  ...
  function substituirSimboloEmObjeto(obj: any, simboloOriginal: string, novoSimbolo: string): any {
  ...
  return (
    <Link href={{ pathname: `/recipe/${item.id}`, params: { recipe: JSON.stringify(substituirSimboloEmObjeto(item, simboloOriginal, novoSimbolo)) } }} asChild>
      <TouchableOpacity
        activeOpacity={0.8}
        onPress={() => console.log("Recipe: ", JSON.stringify(item))}
      ...
      >
        ...
        <View style={styles.detailContainer}>
          ...
          <View style={{ flexDirection: 'row' }}>
            ...
            <View style={[styles.infoContainer, { marginLeft: 19 }]}>
              <Svg width="13.6" height="16" viewBox="0 0 170 200" fill="none">
              ...
              <Text style={styles.infoText}>
                {Math.round(item.nutrients.caloriesKCal)} Kcal
              </Text>
            </View>
